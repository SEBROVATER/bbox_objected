...
def get_cos_between(bbox1: AnyBBox, bbox2: AnyBBox, xc: float, yc: float) -> float:
...
def get_IoU(bbox_1: AnyBBox, bbox_2: AnyBBox) -> float:  # noqa: N802
    ...
    Raises:
        TypeError: if any bbox argument isn't 'AnyBBox' instance

    """
    if not (hasattr(bbox_1, "area") and hasattr(bbox_2, "area")):
        err = "both sources must have 'area' attribute"
        raise TypeError(err)
...
def get_distance(bbox_1: AnyBBox, bbox_2: AnyBBox) -> float:
...
if _NUMPY_AVAILABLE:
    ...
    def non_max_suppression(x1y1x2y2: list | np.ndarray, thr: float) -> list[int | float]:  # noqa: PLR0914
        if len(x1y1x2y2) == 0:
            return []
    ...
    def non_max_suppression(*args, **kwargs) -> None:
        err = "'numpy' is required"
